...
import { MyERC20, MyERC20__factory, MyGovernor, MyGovernor__factory } from '@/types';
import {
  Alert,
  Avatar,
  Badge,
        ...
        if (tx) {
          ...
          if (receipt && receipt.logs && receipt.logs.length > 0) {
            const log = receipt.logs.find(
              (log) =>
                // @ts-expect-error log.fragment type error
                log.fragment.name === 'ProposalCreated'
            );

            // @ts-expect-error log.args type error
...
function CreateProposalForm({
  ...
  onProposalTopicChange: Dispatch<SetStateAction<string>>;
...
function ProposalCard({ proposal, myGovernorContract }: { proposal: Proposal; myGovernorContract: MyGovernor | null }) {
  const [proposalStatus, setProposalStatus] = useState(BigInt(proposal.status));
  // @ts-expect-error proposal.votes type error
  ...
  const handleQueue = async () => {
    try {
      if (myGovernorContract) {
        ...
        await myGovernorContract.queue(proposal.targets, proposal.values, proposal.calldatas, descriptionHash);
  ...
  const handleExecute = async () => {
    try {
      if (myGovernorContract) {
        const descriptionHash = ethers.keccak256(ethers.toUtf8Bytes(proposal.description));
        await myGovernorContract.execute(proposal.targets, proposal.values, proposal.calldatas, descriptionHash);
  ...
  return (
    <Card shadow="sm" padding="lg" radius="md" withBorder>
      ...
      <Group justify="space-between" mt="md" mb="xs">
        <Text fw={500}>{getStatusString(Number(proposalStatus))}</Text>
        ...
        {votesStatus && Number(proposalStatus) != ProposalStatus.Pending && (
          <Stack>
      ...
      {Number(proposalStatus) == ProposalStatus.Active && (
        <Stack>
      ...
      {Number(proposalStatus) == ProposalStatus.Succeeded && <Button onClick={handleQueue}>Queue</Button>}

      {/* ProposalStatusがQueuedのときだけ、Executeボタンを表示 */}
      {Number(proposalStatus) == ProposalStatus.Queued && <Button onClick={handleExecute}>Execute</Button>}
